...
ABREXはPython3によって記述されているため，Python3の実行環境が必要である．
追加で PyYAML パッケージが必要である(`pip install pyyaml`)．
...
$ python abrex.py -p AUTOSAR_MOD_ECUConfigurationParameters.arxml
...
(2.4) XMLファイルの生成
———————————————————————————————————
作成したYAMLファイルを引数として，abrex.pyを実行する．

$ python abrex.py ./sample.yaml
...
$ python abrex.py -i sample.arxml
...
$ python abrex.py -b Com -c AUTOSAR_MOD_ECUConfigurationParameters.arxml
