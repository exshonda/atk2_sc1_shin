...
$
$   パス2のチップ依存テンプレート (Renesas RA + FSP; Cortex-M33 系)
$
$   prc.tf より前に target.tf から取り込まれることで，アーキ層 (prc.tf)
$   と kernel.tf 共通部にチップ固有値を渡す．
$
$   現状値は RA6M5/RA6M4/RA4M2/RA6T2 共通の 96 スロット (BSP_ICU_VECTOR
$   _NUM_ENTRIES = 96) 前提．RA8M1 (128 スロット) 等で広げる必要が
$   出たら MCU_GROUP に応じて分岐する．
$
