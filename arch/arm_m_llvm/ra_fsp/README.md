...
**重要**: 本ディレクトリには `fsp/` サブツリーは存在しない．FSP ソースは
ターゲットごとに `target/<TARGET>/ra/fsp/` に生成される (gitignore 済)．

```
target/ek_ra6m5_gcc/
├── configuration.xml          Smart Configurator のソース (committed; 真値)
├── ra/                        rascc --generate で生成 (gitignored)
│   └── fsp/                   FSP ソース (上流 pack そのまま)
...
│   ├── pin_data.c
│   └── vector_data.{c,h}
└── ... (target 固有ソース; committed)
```

`configuration.xml` を真値とし，clone 後の作業者が `rascc --generate` を
1 回実行すれば `ra/`, `ra_cfg/`, `ra_gen/` がすべて再現される．
...
ここで `$(CHIPDIR) = $(SRCDIR)/arch/arm_m_gcc/ra_fsp`,
`$(FSPDIR) = $(TARGETDIR)/ra/fsp`.
...
`rascc --generate` で生成される FSP 6.4.0 のソース (`target/<T>/ra/fsp/`) は
