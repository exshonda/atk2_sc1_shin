...
| Commit | 内容 |
|---|---|
| (今後) | ARM LLVM への移行: `arm_m_gcc` → `arm_m_llvm`，`ek_ra6m5_gcc` → `ek_ra6m5_llvm`．FSP 同梱撤回．SCI3 (Arduino D0/D1) 採用 |
...
| EK-RA6M5 で Arduino D0/D1 (P303/P302) が SCI3 でない | ボード Rev 違い | ボード silkscreen と [EK-RA6M5 User's Manual](https://www.renesas.com/en/document/mat/ek-ra6m5-v1-users-manual) を確認 |
...
- ユーザが e² studio Smart Configurator を 1 度起動して下記構成のプロジェクトを生成できる:
  ...
  - Pin Configuration: SCI3 UART (P303 = RXD3 / Arduino D0, P302 = TXD3 / Arduino D1) を有効化
...
- **シリアル**: SCI3 を使用．EK-RA6M5 の Arduino-UNO 互換ヘッダ J24 の Pin 0 (RX = P303 = RXD3) / Pin 1 (TX = P302 = TXD3) に接続．115200bps, 8N1, ISR 駆動 RX．外付け USB-Serial 変換アダプタを J24 に接続して使う想定．J-Link OB VCOM (= SCI9) は使用しない．H5 と同様にレジスタ直叩きを基本とし，FSP `r_sci_uart` は使わない (Phase 4 で評価し，FSP に切替えても可)．
- **HW カウンタ**: GPT320 (32-bit) をフリーランニング 1 MHz (PCLKD=100MHz / 100)，GPT321 をワンショットアラーム 1 MHz．H5 の TIM2/TIM5 と同役割．
- **割込み優先度**: Cortex-M33 (4-bit, STM32H5xx と同じ): `tmin_basepri = 0x10`，GPT321 INTPRI=1，SCI3 RXI INTPRI=2，PendSV=0xFF，SVCall=0xE0．
...
├── ek_ra6m5.h                 CPU_CLOCK_HZ, LED ピン, SCI3 ピン定数
├── target_kernel.h            TMIN_INTNO=16, TMAX_INTNO=111, TBITW_IPRI=4 等
├── target_config.c / .h       初期化 (R_BSP_WarmStart 連携 + IELSR 設定 + SCI3 ISR テーブル)
...
4. **シリアル**: `target_config.c` の H5 USART3 部分を SCI3 に置換．`target_serial.h` の `INTNO_SIO` を `g_interrupt_event_link_select` の SCI3_RXI スロット番号に．
...
9. **target_config.c の追加実装**:
   ...
   - **`target_hardware_initialize()`**: BSS 後．下記順序で初期化:
     ...
     3. SCI3 の low-level 初期化 (レジスタ直叩き or `R_SCI_UART_Open`)
...
| EK-RA6M5 の VCOM が SCI3 でない可能性 (ボード Rev 違い) | ユーザーズマニュアルで配線確認．異なれば適切な SCI に変更 | Phase 2-B step 4 |
...
- 主要ファイル:
  ...
  - `target_config.c` (起動経路 §設計判断 (α): `hardware_init_hook` 空，`target_hardware_initialize` で SystemInit + IOPORT + SCI3 + prc_hw_init)
  - `target_serial.c/h` ＋ `target_serial.arxml` (SCI3 RX 割込み，INTNO 暫定 16)
...
- `target_serial.h` `INTNO_SIO` の実値 (`g_interrupt_event_link_select[]` の SCI3_RXI スロット番号 + 16)
...
- SCI3 BRR=26 計算は PCLKB=100MHz 仮定．Smart Configurator の `bsp_clock_cfg.h` で PCLKB 値を確認し，必要なら更新．
