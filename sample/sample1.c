	...
	while (1) {
		...
		if ((command <= (uint8) (0x1fU)) || (command >= (uint8) (0x80U))) {
			...
			case '6':
				/*
				 *  ハードウェアカウンタ動作確認
				 *  MainEvt は MainCycArm (MAIN_HW_COUNTER 上で 10ms 周期)
				 *  でセットされる. 500 回待てば 5 秒経過する想定.
				 */
				{
					TickType	cnt_val;
					uint32		n;
					...
					syslog(LOG_INFO, "HW counter test: 5sec x 4");
					for (n = 1U; n <= 4U; n++) {
						for (t = 0U; t < 500U; t++) {
							WaitEvent(MainEvt);
							ClearEvent(MainEvt);
						}
						(void) GetCounterValue(MAIN_HW_COUNTER,
						                        &cnt_val);
						syslog(LOG_INFO,
						       "%u/4: 5sec elapsed (cnt=%u)",
						       n, cnt_val);
					}
				}
				break;
