...
arch/arm_m_gcc/ra6m5_fsp/README.md   Phase 1 で作成済．Phase 6 で実機検証
                                済バージョンに更新 (動作確認状況追記)
target/ek_ra6m5_gcc/README.md   Phase 2 で作成済．Phase 6 で完成版に更新
...
1. ルート `README.md` 修正:
   - 「ディレクトリ構成」: `target/ek_ra6m5_gcc/` と `obj/obj_ek_ra6m5/` を追加．
...
2. `CLAUDE.md` 修正:
   - 「Architecture layering」表に行追加: chip = `arch/arm_m_gcc/ra6m5_fsp/`, target = `target/ek_ra6m5_gcc/` (RA6M5 + FSP 6.1.0).
...
3. `arch/arm_m_gcc/ra6m5_fsp/README.md` の §9 「バージョン履歴」を Phase 6 完了に更新．§7 「既知の制限」から「コンパイルできない」を削除．
4. `target/ek_ra6m5_gcc/README.md` 完成版を NUCLEO-H563ZI README の章立てで作成:
   - §1 構成
   - §2 開発環境バージョン
...
1. Phase 2 で生成した e² studio プロジェクトを `target/ek_ra6m5_gcc/e2studio/` にコピー．`configuration.xml` を含む．
...
4. `target/ek_ra6m5_gcc/README.md` §9 に取込手順を記載．
...
- [ ] `target/ek_ra6m5_gcc/README.md` が独立したドキュメントとして完結．
- [ ] (任意) e² studio で `target/ek_ra6m5_gcc/e2studio/` を import → Build → Flash → 実機動作 が手順書通りに到達できる．
